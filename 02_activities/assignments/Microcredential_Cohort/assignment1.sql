...
SELECT*        --only for assignment use SELECT*
FROM customer; 
...
WHERE product_id = 9 OR product_id = 4     --there is no PRODUCT ID 9 actually
LIMIT 25;
...
SELECT*, round(quantity * cost_per_quantity,2) AS price    --round to 2 digits
FROM customer_purchases
...
round(sum(cp.quantity*cp.cost_per_quantity),2) AS total_spend    --round to 2 digits
FROM customer_purchases AS cp
INNER JOIN customer AS c
	ON cp.customer_id = c.customer_id
GROUP BY cp.customer_id, c.customer_last_name, c.customer_first_name
HAVING total_spend > 2000.00
...
SELECT*
FROM vendor;                  ---- not sure why, but I must add this ; to make the code run correctly
